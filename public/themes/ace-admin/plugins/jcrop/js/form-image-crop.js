var FormImageCrop = function () {
    ...
    var demo3 = function() {
        ...
        $('#demo3').Jcrop({
          ...
          bgFade: true,
          //allowResize: false,
          minSize: [90, 90], // min crop size
          maxSize: [180, 180], // max crop size
          ...
          jcrop_api = this;
          // Set the location crop from user data
          jcrop_api.animateTo([ crop_x, crop_y, crop_w, crop_h ]);
        ...
        function updatePreview(c)
        {
          if (parseInt(c.w) > 0)
          {
            var rx = xsize / c.w;
            var ry = ysize / c.h;
            // Update hidden input value for image sizes
            $('#crop_x').val(c.x);
            $('#crop_y').val(c.y);
            $('#crop_w').val(c.w);
            $('#crop_h').val(c.h);
        ...
        $('#image-crop-holder #demo3_form').click(function(){
        ...
        //return false;
            ...
            var crop_w = imgHldr.find('#crop_w').val();
            var crop_h = imgHldr.find('#crop_h').val();
            var string = { image: image, path, crop_x: crop_x, crop_y : crop_y, crop_w : crop_w, crop_h: crop_h};
            var params = jQuery.param(string);
            // Send data to crop
            $.ajax({
                method: "GET",
