...
<div class="page-header">
	<h1>Banners <span class="pull-right"><a href="{{ route('admin.banners.create') }}" class="btn btn-warning"><span class="glyphicon glyphicon-pencil"></span> Create</a></span>{{$junked ? ' &raquo; Trashed' :''}}</h1>
</div>
@if($deleted)
<div class="pull-right">
	<a href="{{route('admin.banners.index','path=trashed')}}" title="Restored Deleted"><span class="fa fa-trash"></span> {{ $deleted }} Deleted</a>
</div>
@endif
@if ($rows->count())
<div class="row">
				...
				<tr class="{{ $row->deleted_at ? ' bg-warning' :'' }}">
					...
					<td>
						<div class="btn-group">
							...
							<a data-rel="tooltip" data-original-title="Permanent Delete!" href="{{route('admin.banners.delete', $row->id)}}" class="btn btn-xs btn-danger tooltip-default">
