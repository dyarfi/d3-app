...
<div class="page-header">
	<h1>Portfolios <span class="pull-right"><a href="{{ route('admin.blogs.create') }}" class="btn btn-warning"><span class="glyphicon glyphicon-pencil"></span> Create</a></span>{{$junked ? ' &raquo; Trashed' :''}}</h1>
</div>
@if($deleted)
				...
				<tr class="{{ $row->deleted_at ? ' bg-warning' :'' }}">
					...
					<td>
						<div class="btn-group">
							...
							<a title="Permanent Delete!" href="{{route('admin.blogs.delete', $row->id)}}" class="btn btn-xs btn-danger">
