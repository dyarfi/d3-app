...
<div class="page-header">
	<h1>{{ $mode == 'create' ? 'Create Participant' : 'Update Participant' }} <small>{{ $mode === 'update' ? $row->name : null }}</small></h1>
</div>
<!--form method="post" action="" autocomplete="off"-->
{!! Form::open([
    'route' => ($mode == 'create') ? 'admin.participants.create' : ['admin.participants.update', $row->id]
]) !!}

	<div class="form-group{{ $errors->first('name', ' has-error') }}">
		<label for="name">Name</label>
		<input type="text" class="form-control" name="name" id="name" value="{{ Input::old('name', $row->name) }}" placeholder="Enter the user name.">
	...
	<div class="form-group{{ $errors->first('phone_number', ' has-error') }}">
		...
		{!! Form::text('phone_number',Input::old('phone_number', $row->phone_number),[
	...
	<div class="form-group{{ $errors->first('phone_home', ' has-error') }}">
		...
		{!! Form::text('phone_home',Input::old('phone_home', $row->phone_home),[
	...
	<div class="form-group{{ $errors->first('address', ' has-error') }}">
		...
		{!! Form::textarea('address',Input::old('address', $row->address),[
