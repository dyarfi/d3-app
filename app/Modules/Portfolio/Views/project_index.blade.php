...
<div class="page-header">
	<h1>Projects <span class="pull-right"><a href="{{ route('admin.projects.create') }}" class="btn btn-warning"><span class="glyphicon glyphicon-pencil"></span> Create</a></span>{{$junked ? ' &raquo; Trashed' :''}}</h1>
</div>
@if($deleted)
		...
		<table id="dynamic-table" class="table table-bordered table-hover">
			...
			<tbody>
				...
				<tr class="{{ $row->deleted_at ? ' bg-warning' :'' }}">
					...
					<td>
						@if($row->client)
						<a href="{{route('admin.clients.show',$row->client->id)}}">
							{{ $row->client->name }}
						</a>
						@endif
					</td>
					...
					<td>
						<div class="btn-group">
							...
							<a title="Permanent Delete!" href="{{route('admin.projects.delete', $row->id)}}" class="btn btn-xs btn-danger">
