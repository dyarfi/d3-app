...
<div class="page-header">
	<h1>Portfolios <span class="pull-right"><a href="{{ route('admin.portfolios.create') }}" class="btn btn-warning"><span class="glyphicon glyphicon-pencil"></span> Create</a></span>{{$junked ? ' &raquo; Trashed' :''}}</h1>
</div>
@if($deleted)
<div class="pull-right">
	<a href="{{route('admin.portfolios.index','path=trashed')}}" title="Restored Deleted"><span class="fa fa-trash"></span> {{ $deleted }} Deleted</a>
</div>
@endif
@if ($rows->count())
<div class="row">
				...
				<tr class="{{ $row->deleted_at ? ' bg-warning' :'' }}">
					...
					<td>
						<div class="btn-group">
							...
							<a title="Permanent Delete!" href="{{route('admin.portfolios.delete', $row->id)}}" class="btn btn-xs btn-danger">
